...
const createRandomInteger = (min = 0, max = 2147483646) =>
  Math.floor(Math.random() * (max - min + 1) + min);

const randShade = (hue: number) => ({
  hue: hue,
  saturation: createRandomInteger(50, 70),
  lightness: createRandomInteger(50, 60),
});

const hslString = (color: HSLType) =>
...
const svgGridPattern = (
  ...
  [...Array(Math.floor(height / gridHeight)).keys()]
    .map((y) =>
...
const backgroundSvg = (
  ...
  height: number,
  {
    gridWidth = createRandomInteger(5, 50),
    gridHeight = createRandomInteger(5, 50),
    scheme = randScheme(createRandomInteger(0, 360)),
    seeds = [createRandomInteger(), createRandomInteger()],
  } = {}
) =>
  `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><filter id="noise"><feTurbulence type="turbulence" baseFrequency="0.005" seed="${
    seeds[0]
  }" numOctaves="2" result="turbulence"/><feDisplacementMap in2="turbulence" in="SourceGraphic" scale="30" xChannelSelector="R" yChannelSelector="G"/></filter><filter id="heavy"><feTurbulence type="turbulence" baseFrequency="0.005" seed="${
    seeds[1]
  }" numOctaves="2" result="turbulence"/><feDisplacementMap in2="turbulence" in="SourceGraphic" scale="100" xChannelSelector="R" yChannelSelector="G"/></filter><rect width="${width}" height="${height}" fill="${
    scheme[4]
  }"/><rect filter="url(#heavy)"  width="${width / 2}" height="${height}" x="${
    width / 5
  }" fill="${scheme[2]}"/><rect filter="url(#heavy)" width="${
    width / 2
  }" height="${height}" x="${width / 2}" fill="${scheme[3]}"/>${svgGridPattern(
    width,
    height,
    gridWidth,
    gridHeight,
